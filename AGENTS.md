...
Kho có hook `.claude/hooks/chan-lenh-nguy-hiem.py`, chạy trước mọi lệnh Bash hoặc PowerShell và mọi lần ghi tệp:

- **Chặn hẳn:** `rm -rf`, `Remove-Item -Recurse`, `git push --force`, push thẳng lên `main`/`master`, `git reset --hard`, `git clean -f`, `git branch -D`, `find -delete`, tải mã từ Internet rồi chạy luôn, và một số lệnh khác.
